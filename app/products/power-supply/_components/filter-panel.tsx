...
import { SlidersHorizontalIcon, StoreIcon, ZapIcon } from "lucide-react";
...
import { powerSupplyFilterParsers } from "@/lib/pcbuildwizard/search-params";
...
interface FilterPanelProps {
  merchants: Merchant[];
  // Can be empty if the manufacturers endpoint failed upstream (it's
  // currently unreliable for this category) — the Fabricante fieldset is
  // simply omitted in that case rather than breaking the page.
...
export function FilterPanel({ merchants, manufacturers }: FilterPanelProps) {
  ...
  const [filters, setFilters] = useQueryStates(powerSupplyFilterParsers, {
    shallow: false,
    startTransition,
  });

  const hasManufacturerFilter = manufacturers.length > 0;
