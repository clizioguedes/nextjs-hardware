...
import { GpuIcon, PaletteIcon, SlidersHorizontalIcon, StoreIcon } from "lucide-react";
...
import { videoCardFilterParsers } from "@/lib/pcbuildwizard/search-params";
import type { ColorFeature, Manufacturer, Merchant } from "@/lib/pcbuildwizard/types";
import { FilterFieldset } from "@/app/placas-de-video/_components/filter-fieldset";
...
export function FilterPanel({ merchants, manufacturers, colors }: FilterPanelProps) {
  ...
  const [filters, setFilters] = useQueryStates(videoCardFilterParsers, {
    shallow: false,
    startTransition,
  });

  const hasActiveFilters =
    filters.merchants.length > 0 || filters.manufacturers.length > 0 || filters.colors.length > 0;
    ...
    return (
      <>
        ...
        <FilterFieldset
          title="Cor"
          icon={PaletteIcon}
          options={colors.map((c) => ({ value: c.name, label: c.localName }))}
      ...
      <aside className="hidden w-64 shrink-0 flex-col gap-4 lg:flex">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            onClick={() => setFilters({ merchants: [], manufacturers: [], colors: [] })}
          >
            Limpar
          </Button>
        </div>
